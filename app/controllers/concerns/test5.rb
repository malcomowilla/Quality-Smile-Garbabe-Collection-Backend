 ...
 head :no_content
  ...
  def login
    ActsAsTenant.with_tenant(ActsAsTenant.current_tenant) do
      ...
      if params[:enable_2fa_for_admin].to_s == 'true'
        handle_2fa_login(admin)
      else
        handle_regular_login(admin)
      end
    end
  ...
  def handle_2fa_login(admin)
    unless admin&.authenticate(params[:password])
      render json: { error: 'Invalid Email Or Password' }, status: :unauthorized
      return
    end

    if params[:login_with_otp].to_s == 'true'
      admin.generate_otp(admin)
      send_otp(params[:phone_number], admin.otp, admin.user_name)
      render json: { message: 'sms sent' }, status: :ok
    elsif params[:login_with_otp_email].to_s == 'true'
      handle_email_otp(admin)
    end
  end

  def handle_regular_login(admin)
    unless admin&.authenticate(params[:password])
      render json: { error: 'Invalid Email Or Password' }, status: :unauthorized
      return
    end

    unless admin.account == ActsAsTenant.current_tenant
    ...
    update_admin_status(admin)
    
    render json: admin, serializer: AdminSerializer, status: :accepted
  end

  def handle_email_otp(admin)
    admin.generate_otp(admin)
    update_admin_status(admin)
    AdminOtpMailer.admins_otp(admin).deliver_now
  end

  def update_admin_status(admin)
    admin.update_columns(
      last_login_at: Time.current,
      inactive: false,
      last_activity_active: Time.zone.now
    )
  end

  def set_auth_cookie(token)
    cookies.encrypted.signed[:jwt] = {
      value: token,
      httponly: true,
      secure: true,
      same_site: :strict
  ...
  def register_webauthn
    ...
    # Ensure user can only register one passkey
    if @the_admin.credentials.exists?
      render json: { error: 'A passkey is already registered for this user. For security reasons, only one passkey is allowed.' }, 
    ...
    if @the_admin&.errors&.empty?
      if @the_admin.save
        if @the_admin.webauthn_id.nil?
          @the_admin.update!(webauthn_id: WebAuthn.generate_user_id)
        end
        ...
        session[:registration_user_id] = @the_admin.id

        render json: options
      else
        render json: { error: @the_admin.errors.full_messages }, status: :unprocessable_entity
      end
    else
      render json: { error: @the_admin.errors.full_messages }, status: :unprocessable_entity
    end
  ...
  def check_passkey_status
    admin = Admin.find_by(email: session[:admin_email])
    if admin
  ...
  def verify_webauthn
    begin
      ...
      # Verify the user has a registered passkey
      unless admin.credentials.exists?
        render json: { error: "No passkey registered for this user" }, status: :unauthorized
        return
      end
