      ...
      if @the_admin&.errors&.empty?
        if @the_admin.save
          ...
          options = relying_party.options_for_registration(
            user: { 
              id: Base64.urlsafe_encode64(@the_admin.webauthn_id), 
              name: @the_admin.user_name || @the_admin.email 
            },
            exclude: @the_admin.credentials.map(&:webauthn_id),
            authenticator_selection: { 
              # Require user verification (e.g., PIN, biometric)
              user_verification: 'required',
              # Prevent silent authentication
    ...
    def verify_webauthn
      begin
        ...
        begin
          ...
          # Update admin's session and activity
          ...
          render json: { 
            ...
            message: "The passkey verification failed. Please try again or register a new passkey if the problem persists.",
            details: e.message 
          }, status: :unauthorized
        end
  
      rescue StandardError => e
        Rails.logger.error "WebAuthn Authentication Error: #{e.class} - #{e.message}\n#{e.backtrace.join("\n")}"
        render json: { 
