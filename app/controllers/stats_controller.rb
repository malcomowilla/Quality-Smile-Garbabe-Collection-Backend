class StatsController < ApplicationController
  ...
  #     if work_session.nil? || work_session.started_at.nil?
  #       # Create new session if none exists
  #       work_session = WorkSession.find_or_initialize_by(
  ...
  #       # if current_time - work_session.last_active_at > 5.seconds
  ...
  def app_stats
    begin
      ...
      # Safely find or create today's session
      work_session = WorkSession.find_or_initialize_by(admin_id: @current_user.id, date: Date.current)
      if work_session.new_record? || work_session.started_at.nil?
        work_session.update!(
          started_at: current_time,
          last_active_at: current_time,
          total_time_seconds: 0
        )
      elsif current_time - work_session.last_active_at > 5.seconds
        work_session.update!(last_active_at: current_time)
      end
      ...
      render json: {
        ...
        data: {
          ...
          request_stats: {
            total_requests: Customer.sum(:total_requests),
            total_confirmations: Customer.sum(:total_confirmations),
            confirmation_rate: (Customer.sum(:total_confirmations).to_f / Customer.sum(:total_requests) * 100).round(2)
          },
          session_start: work_session.started_at,
          last_active: work_session.last_active_at,
          debug_info: {
      ...
      render json: { status: 'error', message: e.message, backtrace: e.backtrace }, status: :internal_server_error
    end
  end
